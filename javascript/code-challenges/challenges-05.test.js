...
const addPurchases = (arr) => {
    ...
    let total = arr.reduce((acc, current) => {
        return (acc += current.purchasePrice);
    }, 0);
    return total;
};
...
const returnNames = (arr) => {
    // Solution code here...
    let total = arr.reduce((acc, currant) => {
        acc.push(currant.name);
        return acc;
    }, []);
    return total;
};
...
const reversedString = (str) => {
    ...
    let reverseArr = [];
    for (var i = splittedArr.length - 1; i >= 0; i--) {
        reverseArr.push(splittedArr[i]);
    }
    let newString = reverseArr.reduce((acc, current) => {
        return (acc += current);
    }, "");
    return newString;
};
